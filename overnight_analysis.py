...
API_KEY = os.environ.get("LN_HISTORY_API_KEY", "mQd2frqAaMts5wwzEgHrdhF7y")
BACKEND_URL = os.environ.get("LN_HISTORY_BACKEND_URL", "http://localhost:5050")

_start = os.environ.get("START_DATE", "2018-06-01")
_end = os.environ.get("END_DATE", "2020-09-01")
...
def compute_graph_stats(G: nx.DiGraph, d: date) -> Dict:
    ...
    edges_disabled = sum(
        1 for *_, a in G.edges(data=True) if a.get("has_update") and _channel_disabled(a.get("channel_flags", "00"))
    )
    ...
    return {
        ...
        "largest_component_pct": (
            round(len(components[0]) / G.number_of_nodes() * 100, 2) if G.number_of_nodes() and components else 0.0
        ),
    }
...
def phase2_betweenness(dates: List[date], top_n: Optional[int] = None) -> None:
    """
    ...
    for d in dates:
        ...
        bc: Dict[str, float] = nx.betweenness_centrality(H, k=k, normalized=True, seed=42)
        ...
        with open(out, "w", newline="", encoding="utf-8") as f:
            ...
            for rank, (node_id, score) in enumerate(rows, start=1):
                a = H.nodes[node_id]
                addresses = "; ".join(
                    f"{addr.get('addr', '')}:{addr.get('port', '')}" for addr in a.get("addresses", [])
                )
                writer.writerow(
                    [
                        rank,
...
if __name__ == "__main__":
    ...
    log.info("=" * 60)
    log.info("Phase 2: computing betweenness centrality")
    log.info("=" * 60)
    phase2_betweenness(dates)
